...
SCRIPT_PATH="$PROJECT_ROOT/plugins/claude-code-quality-gate/scripts/quality-gate-status.sh"
...
if [[ ! -f "$SCRIPT_PATH" ]]; then
    ...
    echo "Directory contents of $PROJECT_ROOT/plugins/claude-code-quality-gate/scripts:"
    ls -la "$PROJECT_ROOT/plugins/claude-code-quality-gate/scripts" || true
    exit 1
fi
